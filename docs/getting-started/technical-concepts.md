...
Để tận dụng tối đa Cổng thông tin nhà phát triển Cardano, bạn phải có kinh nghiệm lập trình và hiểu biết cơ bản về các khái niệm blockchain như [UTxO](#unspent-transaction-output-utxo), [transactions](#transactions), [addresses](#addresses), [key derivation](#key-derivation), và [networking](#networking).
...
Key Derivation là quá trình mà ví sử dụng để chuyển một cụm từ dễ nhớ đến toàn bộ các khóa và địa chỉ mà ví kiểm soát.
