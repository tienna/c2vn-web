...
Trước khi chúng tôi viết ra chính sách đúc tiền đầu tiên, hãy nhớ lại ngắn gọn cách thức hoạt động của xác thực.
...
Tập lệnh xác thực đó, dưới dạng đầu vào, dữ liệu, đến từ UTxO, trình mua lại, đến từ đầu vào và ngữ cảnh.
...
Trường khác là  `TxInfo` chứa tất cả thông tin ngữ cảnh về giao dịch.
...
Trong khi các tập lệnh xác thực có ba đầu vào - dữ liệu, trình đổi và ngữ cảnh, các tập lệnh chính sách đúc tiền này chỉ có một đầu vào - ngữ cảnh. Và đó là bối cảnh giống như chúng ta đã có trước đây - `ScriptContext`. Sẽ không có ý nghĩa gì nếu có dữ liệu, vì nó thuộc về UTxO và sẽ không có ý nghĩa gì nếu có người mua lại vì nó thuộc về tập lệnh xác thực. Chính sách đúc tiền thuộc về bản thân giao dịch, không phải đầu vào hoặc đầu ra cụ thể.
...
Hãy viết một chính sách đúc tiền đơn giản.
...
húng tôi cũng đã thấy phiên bản cấp thấp trong đó chúng tôi có ba đối số `Data` và trả về `Unit`. Và chúng tôi thấy rằng có thể có các đối số bổ sung trước số liệu, nếu chúng tôi viết một tập lệnh được tham số hóa.

Chúng ta cũng có thể có các kịch bản chính sách đúc tiền được tham số hóa và chúng ta sẽ thấy điều đó trong một ví dụ sau. Nhưng trước tiên chúng ta sẽ xem xét một cái không được tham số hóa.
...
Bây giờ chúng tôi có một chính sách, chúng tôi có thể nhận được một ký hiệu tiền tệ từ chính sách.
...
Và điều này hoàn thành phần on-chain, cho việc đúc tiền đơn giản này. Nhưng để dùng thử và tương tác với nó, chúng ta cần một phần off-chain.
...
Như mọi khi, chúng ta có `BlockchainActions` cung cấp cho chúng tôi quyền truy cập vào những thứ chung chung như lấy khóa công khai của riêng bạn. Và ở đây, chúng tôi đã thêm một điểm cuối `mint` bằng cách sử dụng toán tử cấp kiểu mà chúng tôi đã thấy trước đây.
...
Trong quá khứ, chúng tôi đã không đi sâu vào chi tiết về phần ngoài chuỗi của hợp đồng. Nhưng, như bây giờ chúng ta đã biết về `Contract monad`   từ bài giảng trước, chúng ta đã sẵn sàng đi vào chi tiết hơn nhiều.

Nhớ lại rằng `Contract` monad Nhận 4 tham số.

Đầu tiên là đơn nguyên của người viết cho phép chúng ta sử dụng một hàm `tell`. Bằng cách để tham số này bằng một số nhỏ w, chúng tôi chỉ ra rằng chúng tôi sẽ không sử dụng tham số này - chúng tôi sẽ không có hàm `tell` với bất kỳ trạng thái nào.
...
Cuối cùng, tham số cuối cùng là kiểu trả về, và hợp đồng của chúng ta sẽ chỉ có kiểu trả về Unit.

Bây giờ phần thân hàm, như là `Contact` là một monad, Chúng ta có thể sử sùng`do`.
...
Một trong những mục đích chính của `Contract` monad là xây dựng và gửi các giao dịch. Con đường mà nhóm Plutus đã thực hiện là cung cấp một cách để chỉ định các ràng buộc của giao dịch mà bạn đang xác định. Sau đó, các thư viện Plutus sẽ đảm nhận việc xây dựng giao dịch chính xác (nếu có thể). Điều này trái ngược với việc yêu cầu chỉ định tất cả các đầu vào và đầu ra theo cách thủ công, điều này sẽ rất tẻ nhạt vì nhiều yêu cầu, chẳng hạn như gửi tiền lẻ trở lại ví gửi, thường giống nhau.
...
Tất cả các hàm `submitTx` đều nhận các điều kiện khai báo này mà giao dịch phải thỏa mãn, và sau đó chúng cố gắng xây dựng một giao dịch đáp ứng các điều kiện đó. Trong trường hợp của chúng tôi, điều kiện duy nhất là chúng tôi muốn giả mạo giá trị.
...
Tuy nhiên, ký hiệu tiền tệ chỉ là mã băm của tập lệnh chính sách. Để tự chạy tập lệnh, nó phải được bao gồm trong giao dịch. Có nghĩa là, trong bước xây dựng của giao dịch, khi thuật toán nhìn thấy ràng buộc `mustForgeValue`,  nó biết rằng nó phải đính kèm tập lệnh chính sách tương ứng vào giao dịch.

Để cho thuật toán biết tập lệnh chính sách ở đâu, chúng tôi có thể đưa ra các gợi ý và đây là các bản tra cứu. Có thể sử dụng nhiều cách tra cứu khác nhau - bạn có thể cung cấp UTxO, tập lệnh trình xác thực và, như chúng tôi làm ở đây, bạn có thể cung cấp tập lệnh chính sách tiền tệ.

Trong trường hợp của chúng tôi, điều duy nhất chúng tôi cần cung cấp khi tra cứu là chính sách mà chúng tôi đã xác định trước đó trong tập lệnh.
...
Hầu hết các hàm ràng buộc đều hướng tới việc sử dụng một tập lệnh trình xác nhận cụ thể. Thông thường, bạn có tình huống rằng bạn đang làm việc trên một hợp đồng thông minh cụ thể. Và hợp đồng thông minh cụ thể đó có mức dữ liệu và kiểu người mua, và hầu hết các chức năng ràng buộc là tham số trong kiểu dữ liệu và kiểu người mua. Trong trường hợp đó, bạn có thể sử dụng trực tiếp kiểu dữ liệu mà không cần phải chuyển đổi nó sang kiểu Plutus
`Datum`.
...
Ngoài ra, trong cùng một dòng, chúng ta thấy một ràng buộc đơn nguyên, vì vậy chúng ta biết rằng đây là một hành động đơn nguyên xảy ra trong `Contract` monad. Lý do cho điều này là, để tra cứu, chẳng hạn như các UTxO của chúng ta, hàm `submitTxConstraintsWith` phải sử dụng sức mạnh siêu việt của `Contract` đơn nguyên, đó là để truy cập `BlockchainActions`.
...
Cuối cùng, chúng ta cần thêm một số bản soạn sẵn để xác định điểm cuối của chúng ta, để có thể thực thi hàm `mint`, ví dụ, trong playground.
...
Chúng tôi xác định một hợp đồng khác `endpoints`, và đó luôn là tên của hợp đồng mà playground sẽ chạy. Vì vậy, nếu bạn muốn thử nghiệm điều gì đó trong playground, bạn luôn cần một thứ được gọi là `endpoints`.
...
Vì `mint'` bằng cách nào đó chúng ta phải có được `MintParams` và để sử dụng `endpoint`. Hàm `endpoint` cung cấp một tham số cho Contract. Khi tham số của `MintParams` được cung cấp, chúng tôi sử dụng ràng buộc đơn nguyên để gọi `mint` với ccacs đối số đó.
...
Chúng tôi nhớ rằng `scriptContextTxInfo` từ ngữ cảnh có chứa danh sách tất cả các bên ký kết của giao dịch. Vì vậy, chúng tôi có thể sử dụng điều này để xem liệu người ký được yêu cầu có phải là một trong số họ hay không.
...
We don\'t need to extend the `MintParams` Chúng tôi không cần mở rộng `MintParams` kiểu dữ liệu cho Off-chain. Ví muốn đúc hoặc đốt tiền tệ có thể ký bằng mã băm khóa công khai của chính nó. Đây là chữ ký duy nhất mà ví có thể cung cấp và nó có khả năng tự tra cứu.

Chúng tôi sẽ thực hiện một thay đổi đối với tên của lược đồ cho rõ ràng. Tất nhiên, chúng tôi cũng sẽ cập nhật tên này bất cứ nơi nào nó xuất hiện trong tập lệnh hợp đồng.t
script.
...
`Contract a` thành `Contract b`. Trong trường hợp này, chúng ta có thể tận dụng điều đó bằng cách sử dụng hàm `pubKeyHash` như là (a -> b) của hàm fmap và sẽ chuyển `Contract pubKey` thành `Contract pubKeyHash`,và sau đó có thể hàm grab để lấy giá trị.
...
Được rồi, bây giờ chúng ta hãy cập nhật dòng tra cứu để chuyển trong hàm băm khóa công khai.
...
Ý tưởng là sử dụng UTxOs. UTxO là duy nhất. UTxO là đầu ra của một giao dịch và số nhận dạng duy nhất của nó là ID giao dịch và chỉ mục của nó trong danh sách các đầu ra từ giao dịch đó.
...
Vì vậy, để tạo NFT, chúng tôi sẽ cung cấp một UTxO cụ thể làm tham số cho chính sách đúc tiền và trong chính sách này, chúng tôi sẽ kiểm tra xem giao dịch có sử dụng UTxO này hay không. Và, như chúng tôi vừa lưu ý, một khi UTxO đó đã được tiêu thụ, nó không bao giờ có thể được tiêu thụ lại.

### Ví dụ 3 - NFT

Chúng tôi bắt đầu với một bản sao của ví dụ trước `Signed` avà chúng tôi sẽ gọi `NFT`.

Vì vậy, hãy biến chính sách đã ký thành một chính sách NFT thực sự.
...
Ở đây, chúng tôi sử dụng anyhàm để xem liệu có bất kỳ `txInInfoOutRefs` nào trong `txInfoInputs` từ `TxInfo` trường của ngữ cảnh khớp với UTxO mà chúng tôi đang xác thực hay không.
...
Vì vậy, chúng ta có đủ để hoàn thành việc viết chính sách của chúng tôi? Hãy xem những gì chúng tôi có.
...
Và chúng tôi cần một điều kiện thứ hai để kiểm tra xem chúng tôi có đúc chỉ một đồng xu cụ thể này hay không.
...
Nhưng chúng tôi vẫn còn một vấn đề cần giải quyết - chúng tôi cần biết ký hiệu tiền tệ là gì. Cho rằng biểu tượng tiền tệ là một hàm băm của chính sách, có vẻ như chúng ta đang gặp vấn đề về con gà và quả trứng.
...
Bây giờ, chúng tôi có thể hoàn thành chính sách của mình.
...
Và chúng tôi sẽ cập nhật bảng nấu ăn của chúng tôi.
...
một `AddressMap` là map trong đó  keys là `TxOutRef`s giá trị là `TxOutTx`.
...
Nếu chúng tôi chuyển chức năng này đến địa chỉ của chính chúng tôi thì các khóa của bản đồ này sẽ là các UTxO thuộc về chúng tôi. Chúng tôi chọn cái nào trong số này không quan trọng. Vì vậy, miễn là chúng ta sở hữu ít nhất một UTxO, chúng ta đã tốt.

Bước đầu tiên là tìm địa chỉ của chính chúng tôi. Chúng tôi biết cách tìm khóa công khai của riêng mình và, với điều này, chúng tôi có thể sử dụng chức năng này `pubKeyAddress` để lấy địa chỉ của mình.
...
Chúng tôi chỉ cần một cái - chúng tôi không quan tâm cái nào. Chúng tôi sẽ viết một câu lệnh trường hợp sẽ ghi lại lỗi nếu chúng tôi không có UTxO có sẵn hoặc sẽ sử dụng UTxO đầu tiên trong danh sách tiếp tục với mã giả mạo.

Thay đổi đầu tiên là chỉ định 1thay vì chỉ định `mpAmount`, vì chúng tôi muốn có chính xác 1 đồng xu được đúc.
...
Làm thế nào để chúng ta kết hợp các ràng buộc của `mustForgeValue` và `mustSpendPubKeyOutput`? `Contraints`không tạo thành a `Monoid`, nhưng chúng có tạo thành a Semigroup, và sự khác biệt chỉ là ở chỗ `Semigroup` chúng ta không có `mempty`, phần tử trung tính. Chúng tôi vẫn có thể kết hợp chúng với toán tử `<>`.
...
Vì vậy, hãy cập nhật tra cứu của chúng tôi.

``` {.haskell}
lookups = Constraints.monetaryPolicy (policy oref tn) <> Constraints.unspentOutputs utxos
```

Một cái gì đó chúng ta cần phải làm trước khi kịch bản này sẽ chạy là phải nhập khẩu các nhà điều hành `<>` cho Semigrouptừ Haskell Prelude tiêu chuẩn, như chúng ta đã loại trừ một cách rõ ràng từ các mudule `PlutusTx.Prelude`.
